...
Terminal arm-output classification (`started`, `attached`, a benign `deferred` daemon handoff, or `FAILED`) remains defense in depth for the manual recovery path.
...
While a live, identity-matched away-mode daemon owns supervision (`fm_afk_daemon_owns_supervision`), every arm mode yields before stopping or starting anything and prints `watcher: deferred - away-mode daemon owns supervision`, because the daemon's own one-shot watcher child is then the home's one singleton.
A fresh entry marker that makes `fm_afk_launch_in_progress` true yields the same way before the daemon takes over, so an arm cannot race that handoff.
The persistent adapters treat that typed line as a benign no-op and never retry, surface, or deliver from it.
For the same reason the daemon start path itself retires a watcher already live in this home (only a live pid whose recorded home, watcher path, and process identity match) before it forks its own child, so an arm watcher that was live at entry hands over instead of colliding on the singleton lock.
