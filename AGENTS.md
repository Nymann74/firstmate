...
state/               runtime records and signals; gitignored
  ...
  .afk-launching     transient away-mode entry marker: bin/fm-afk-launch.sh writes its creation epoch before state/.afk and clears it once a daemon owns supervision (or on failure/interrupt); the arm gate yields while it is fresh so an in-progress launch is never raced, but fm_afk_launch_in_progress (bin/fm-wake-lib.sh) ignores and clears a marker older than FM_AFK_LAUNCHING_MAX_SECS (default 300) when no live daemon owns supervision
